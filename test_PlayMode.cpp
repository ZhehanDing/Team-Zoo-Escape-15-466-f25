...
Load< SkeletonBuffer > skeletons(LoadTagDefault, []() -> SkeletonBuffer const * {
	SkeletonBuffer const *ret = new SkeletonBuffer(data_path("../animation/test.skel"));
	return ret;
});

Load< AnimationBuffer< Skeleton::BoneTransform > > animations(LoadTagDefault, []() -> AnimationBuffer< Skeleton::BoneTransform > const * {
	AnimationBuffer< Skeleton::BoneTransform > const *ret = new AnimationBuffer< Skeleton::BoneTransform >(data_path("../animation/test.anim"));
	return ret;
});
	...
	return new Scene(data_path("test.scene"), [&](Scene &scene, Scene::Transform *transform, std::string const &mesh_name){
		...
		if (scene.drawables.size() == 2) {
			...
			if (mesh_name == "Beta_Surface") {
				...
				back.transform->position += glm::vec3(0.f);
				//back.transform->rotation = glm::rotate(back.transform->rotation, glm::radians(45.f), {0.f, 0.f, 1.f});
			}
			else {
				back.transform->parent = front.transform;
				front.transform->position += glm::vec3(0.f);
				//front.transform->rotation = glm::rotate(back.transform->rotation, glm::radians(45.f), {0.f, 0.f, 1.f});
			}
