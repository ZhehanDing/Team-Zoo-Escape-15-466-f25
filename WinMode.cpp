...
WinMode::WinMode() : scene(*menu_scene_deferred) {
	SDL_SetWindowRelativeMouseMode(Mode::window, false);
	overlay.add_element("Title", 
		glm::vec2(0.f, 125.f), 
		glm::vec2(1920.f, 750.f) * .5f,
		glm::vec4(1.f),
		ui_textures->find("Win")->second
	);
...
void WinMode::draw(glm::uvec2 const &drawable_size) {
	...
	win_fb.resize(drawable_size);

	glBindFramebuffer(GL_FRAMEBUFFER, win_fb.objects_fb); // bind the objects (G-buffer) framebuffer as render target
	...
	glBindFramebuffer(GL_FRAMEBUFFER, win_fb.lights_fb); // bind lights framebuffer (output accumulation texture)
	...
	glBindTexture(GL_TEXTURE_2D, win_fb.position_tex); // bind world-space position G-buffer to unit 0
	glActiveTexture(GL_TEXTURE1); // select texture unit 1
	glBindTexture(GL_TEXTURE_2D, win_fb.normal_roughness_tex); // bind normal+roughness G-buffer to unit 1
	glActiveTexture(GL_TEXTURE2); // select texture unit 2
	glBindTexture(GL_TEXTURE_2D, win_fb.albedo_tex); // bind albedo (color) G-buffer to unit 2
	...
	glBindTexture(GL_TEXTURE_2D, win_fb.output_tex);  // Show final lighting result

	// if (show == ShowOutput) {
	// 	glBindTexture(GL_TEXTURE_2D, win_fb.output_tex);  // Show final lighting result
	// } else if (show == ShowPosition) {
	// 	glBindTexture(GL_TEXTURE_2D, win_fb.position_tex);  // Show world-space positions
	// } else if (show == ShowNormalRoughness) {
	// 	glBindTexture(GL_TEXTURE_2D, win_fb.normal_roughness_tex);  // Show normals and roughness
	// } else if (show == ShowAlbedo) {
	// 	glBindTexture(GL_TEXTURE_2D, win_fb.albedo_tex);  // Show surface colors and textures
	// }
