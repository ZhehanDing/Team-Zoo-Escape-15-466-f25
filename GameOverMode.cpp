...
GameOverMode::GameOverMode() : scene(*menu_scene_deferred) {
    ...
    overlay.add_interaction("Play Again Button", []() -> void { 
...
bool GameOverMode::handle_event(SDL_Event const &evt, glm::uvec2 const &window_size) {
...
void GameOverMode::update(float elapsed) {
}

void GameOverMode::draw(glm::uvec2 const &drawable_size) {
	//11/23 Update
	...
	game_over_fb.resize(drawable_size);

	glBindFramebuffer(GL_FRAMEBUFFER, game_over_fb.objects_fb); // bind the objects (G-buffer) framebuffer as render target
	...
	glBindFramebuffer(GL_FRAMEBUFFER, game_over_fb.lights_fb); // bind lights framebuffer (output accumulation texture)
	...
	glBindTexture(GL_TEXTURE_2D, game_over_fb.position_tex); // bind world-space position G-buffer to unit 0
	glActiveTexture(GL_TEXTURE1); // select texture unit 1
	glBindTexture(GL_TEXTURE_2D, game_over_fb.normal_roughness_tex); // bind normal+roughness G-buffer to unit 1
	glActiveTexture(GL_TEXTURE2); // select texture unit 2
	glBindTexture(GL_TEXTURE_2D, game_over_fb.albedo_tex); // bind albedo (color) G-buffer to unit 2
	...
	glBindTexture(GL_TEXTURE_2D, game_over_fb.output_tex);  // Show final lighting result

	// if (show == ShowOutput) {
	// 	glBindTexture(GL_TEXTURE_2D, game_over_fb.output_tex);  // Show final lighting result
	// } else if (show == ShowPosition) {
	// 	glBindTexture(GL_TEXTURE_2D, game_over_fb.position_tex);  // Show world-space positions
	// } else if (show == ShowNormalRoughness) {
	// 	glBindTexture(GL_TEXTURE_2D, game_over_fb.normal_roughness_tex);  // Show normals and roughness
	// } else if (show == ShowAlbedo) {
	// 	glBindTexture(GL_TEXTURE_2D, game_over_fb.albedo_tex);  // Show surface colors and textures
	// }
