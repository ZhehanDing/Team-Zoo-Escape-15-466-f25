...
void PlayMode::draw(glm::uvec2 const &drawable_size) {
	...
	if (focus_mode) {
		glClearColor(1.0f, 1.0f, 1.0f,
					 1.0f); // stark white background for high contrast
	} else {
		...
		if (clip.w > 0.0f) {
			...
			for (int dy = -radius; dy <= radius; dy += step) {
				for (int dx = -radius; dx <= radius; dx += step) {
					int px = int(sx) + dx;
					int py = int(sy) + dy;
					if (px < 0 || py < 0 || px >= int(drawable_size.x) ||
						py >= int(drawable_size.y))
						continue;

					float depth_sample = 1.0f;
					glReadPixels(px, py, 1, 1, GL_DEPTH_COMPONENT, GL_FLOAT, &depth_sample);

					// If the sample depth is *in front of* the enemy (smaller), that
					// point is blocked:
					if (depth_sample + eps < enemy_depth)
						occluded++;
					total++;
				}
			}
			// "Fully blocked" ≈ all tested points occluded:
			if (total > 0 && occluded == total)
				enemy_visible = false;
		} else {
			// behind camera
			enemy_visible = false;
	...
	{ // use DrawLines to overlay some text:
		...
		lines.draw_text("WASD moves character. Right click to stalk the human "
						"visitor to learn how human walks...",
		...
		lines.draw_text(
			"WASD moves character. Right click to stalk the human visitor to learn "
			"how human walks...",
